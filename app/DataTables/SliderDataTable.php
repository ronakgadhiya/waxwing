    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('img',function ($row) {
                if($row['img'] != null ){
                    return '<img src="/uplods/slider/'.$row['img'].'" class="img-fluid rounded" style="height: 100px;width: 100px;" >';
                }else{ 
                    return '';
                }
            })
            ->editColumn('status',function ($row) {
                if($row['status'] == 1 ){
                    return '<div class="form-check form-switch"><input class="form-check-input" onclick="changeStatus(event.target,'.$row['id'].',\'Slider\');" type="checkbox" checked=""></div>';
                }else{ 
                    return '<div class="form-check form-switch"><input class="form-check-input" onclick="changeStatus(event.target,'.$row['id'].',\'Slider\');" type="checkbox"></div>';
                }
            })
