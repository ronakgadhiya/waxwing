...
class BlogDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('status',function ($row) {
                if($row['status'] == 1 ){
                    return '<div class="form-check form-switch"><input class="form-check-input" onclick="changeStatus(event.target,'.$row['id'].',\'Blog\');" type="checkbox" checked=""></div>';
                }else{ 
                    return '<div class="form-check form-switch"><input class="form-check-input" onclick="changeStatus(event.target,'.$row['id'].',\'Blog\');" type="checkbox"></div>';
                }
            })
    ...
    {
        $model = Blog::with(['category'])->select()->orderBy('id','desc');
        ...
        if (request()->get('category', false)) {
            $model->where('categorie_id', 'like', "%" . request()->get("category") . "%");
        }
